...
The `ILlmClient` abstraction, contracts (`Ai/Contracts/`), `StubLlmClient`, `CopyGenService`, and the `Ai:Mode` single-binding DI all shipped exactly as decided here. **One deviation, by the project owner's call:** the live `AnthropicLlmClient` is implemented as a **typed `HttpClient` on the Anthropic Messages API** (`POST /v1/messages`, `anthropic-version: 2023-06-01`, System.Text.Json contract mapping) + `AddStandardResilienceHandler()` — **not** the community `Anthropic.SDK` NuGet named above. Rationale: a stable wire contract with no SDK-version coupling, more transferable learning (resilience + protocol + serialization), and it matches the `IHttpClientFactory` + Polly wiring in CODING_STANDARDS § Resilience. Consequence: the `Directory.Build.targets` guard against `using Anthropic.SDK` is moot (no such namespace is referenced); the abstraction's value — clean seam, one shipping provider, stub fallback — is unchanged, and adding `OpenAiLlmClient` later is still a localized change behind `Ai:Provider`. The same approach was used for the Phase-4 sentiment provider (`AzureTextAnalyticsAdapter` on the Azure AI Language REST API instead of the `Azure.AI.TextAnalytics` SDK). See `docs/PHASE_4_SCOPE.md` §19.
