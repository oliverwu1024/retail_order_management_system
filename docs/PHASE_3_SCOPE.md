...
- **Job B (frontend/product):** the **12+ hand-built primitives** — Phase 3 added `DataTable`/`Modal`/`EmptyState` (→ 13; `Drawer`/`Checkbox`/`Tabs` deferred as not-yet-needed, see §16.1) — "compose, not invent" (component-library bullet); the **role-driven AdminShell** showing different nav per role (the literal "store managers, staff, administrators" bullet); **Vitest + Playwright + axe** E2E (accessible, tested UI / deployment-success bullet).
...
### 16.1 Post-Phase-3 code-review reconciliation (as-built)

A 7-dimension adversarial review at phase-end found **0 critical, 1 high, 1 medium, ~15 low** (24 confirmed/partial after verification). All actionable findings were fixed; the scope-fidelity items below are reconciled here rather than built speculatively.

- **Component-library primitives (review L13).** Phase 3 added `DataTable`, `Modal` (`dialog`), and `EmptyState`, bringing the hand-built library to **13 primitives** (meets the ≥12 bar). `Drawer` / `Checkbox` / `Tabs` were **not** built: no tabbed or drawer screen emerged, and the admin forms use the project's accessible **native `Select`** + a single native publish checkbox — consistent with the deliberate "a native control is the most accessible + needs no dependency" decision behind `Select`. They're **deferred** until a screen genuinely needs them, rather than shipped as unused primitives (which a quality review would flag as dead code). The §15 Job-B bullet rests on the 13 built-and-used primitives.
- **`FilterPanel` lift (review L14).** It stays in `features/storefront` (shared by `CatalogPage` + `AdminProductsPage`) because it's coupled to `useCategoriesQuery`. A generic lift into `components/ui` (categories passed as a prop) is **deferred** until a third consumer needs it; the current cross-feature import is a known, accepted trade-off.
- **Inventory-adjust UI location (review L17).** Ships under the **product variant editor** (Products → open a product → Variants → Adjust); the `/admin/inventory` sidebar item points there. A standalone inventory dashboard is a later enhancement (it would need a new "list inventory" endpoint).
- **Refund/restock audit completeness (review L4/L5/L6) — ✅ fixed.** Set-based `ExecuteUpdate` writes on the refund path now emit explicit `IAuditWriter` rows: `Restocked` per variant, `RefundClaimed`/`RefundClaimReleased` on the claim/release, and the named `Refund` row is recorded once at the real transition with the true prior status. Closes the §3.2 / §16 obligation for the refund flow.
- **Inventory-adjust reserved-aware guard (review M1) — ✅ fixed.** `AdjustAsync` rejects when on-hand would drop below `Reserved` (Available < 0), not merely below zero.
- **Admin E2E + named tests (review H1/L15/L16) — ✅ fixed.** Added the admin Playwright spec (login → create product → mark shipped + an admin a11y scan), a forged-Customer-JWT → 403 integration test, and a concurrent-ship → 409 (RowVersion) test.
- **Date-range + status-filter validation (review L1/L2/L9) — ✅ fixed.** `DateRangeGuard` rejects a reversed `[from,to]` (422) on the workbench/audit/report; the report caps the span at 366 days; `ParseStatus` uses `Enum.IsDefined`.
