...
**The mechanism (resume-gold complexity to get right):**
- In **`SavingChanges`**: snapshot each monitored `ChangeTracker` entry's `State`, `OriginalValues`, and `CurrentValues` into a per-context buffer. Original values must be read pre-save.
- In **`SavedChanges`** (post-save, so DB-generated `Id`/`OrderNumber`/`RowVersion` are populated for `AfterJson`): serialize before/after, build `AuditLog` rows, and persist them. AuditLog is **not** in the allowlist, so writing it does not recurse. Wrap within the ambient transaction when one exists.
- **PII:** serialize a per-entity field allowlist (or redact sensitive fields — `GuestEmail`, address snapshots) before writing JSON; CODING_STANDARDS flags email as sensitive, and AuditLog is admin-readable.
...
- **PII in AuditLog JSON** — must redact sensitive fields (`GuestEmail`, addresses) in the snapshot serializer; treat as a Chunk-0 correctness item, not polish.
