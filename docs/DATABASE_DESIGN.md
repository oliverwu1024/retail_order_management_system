...
| 5B | `0011_order_anomaly` | OrderAnomaly (Score `decimal(8,3)`, Reason, DetectedAt, Acknowledged) + `IX_OrderAnomaly_OrderId`, `IX_OrderAnomaly_Acknowledged_DetectedAt`; FK → Order (**Cascade**). **As-built:** Phase 5B was split again into **anomaly first** (this migration) + **demand forecasting** (next), so the chat-era design label `0011_forecast_anomaly` is superseded — anomaly shipped alone as `<ts>_0011_order_anomaly`. PK is client/EF-generated (no `newsequentialid()`); `DetectedAt` is `datetimeoffset`; there is **no** `AcknowledgedBy/At` (the acknowledge rides `UpdatedBy/At`). See PHASE_5B_SCOPE §4 / §18. |
| 5B (forecast) | `0012_demand_forecast` | DemandForecast (Horizon, ForecastedQty, Lower/UpperBound `decimal(10,2)`, Confidence `decimal(4,3)`, ModelVersion, GeneratedAt) + ReorderHint (RecommendedOrderQty, Reasoning, GeneratedAt, Dismissed) + `IX_DemandForecast_ProductVariantId_GeneratedAt`, `IX_ReorderHint_ProductVariantId_Dismissed_RecommendedOrderQty`; FK → ProductVariant (**Cascade**). As-built (PHASE_5B_FORECAST_SCOPE §4): client/EF GUID PKs, `datetimeoffset`; the forecaster is pure-C# **Holt-Winters**, not ML.NET SSA (MKL/`libiomp5` Linux dep — **ADR-0012**); LowerBound/UpperBound are a quadrature-propagated, ≥0-floored 80% band. |
